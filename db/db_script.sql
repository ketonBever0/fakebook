...
CREATE SEQUENCE FAKEBOOK.FORBIDDEN_EXPRESSIONS_ID_SEQ INCREMENT BY 1 MINVALUE 1 MAXVALUE 9999999999999999999999999999 NOCYCLE NOCACHE ORDER ;
...
CREATE OR REPLACE TRIGGER FAKEBOOK.EXISTING_MEMBERSHIP_T
BEFORE INSERT ON FAKEBOOK.USER_GROUPS
FOR EACH ROW
DECLARE
	match_count NUMBER;
BEGIN
	SELECT COUNT(id)
	INTO match_count
	FROM FAKEBOOK.USER_GROUPS
	WHERE user_id = :NEW.user_id AND group_id = :NEW.group_id;

	IF match_count > 0 THEN
		RAISE_APPLICATION_ERROR(-20001, 'MEMBERSHIP_ALREADY_EXISTS');
	END IF;
END;